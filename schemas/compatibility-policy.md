...
| Reasoning trace | `prediction.reasoning_trace.narrative` and `prediction.reasoning_trace.causal_graph.{nodes,edges}`. | Runtime `ForecastOutput` stores `reasoning`, `logical_trace`, and `logical_edges`; `xrtm-data` accepts governance `reasoning_trace` input and exposes a read-only `reasoning_trace` alias. | New schema examples must use `reasoning_trace`. Runtime package examples may show `logical_trace`/`logical_edges` only when documenting package internals. |
...
| Ground truth / resolution | Evaluation-owned `ForecastResolution(question_id, outcome, resolved_at, metadata)` and evaluator `ground_truth` inputs. | None in the Forecast Object schema. | Do not add resolved outcomes to forecast payloads. Outcomes remain post-forecast evaluation data. |
...
- `reasoning_trace` -> `reasoning`/`logical_trace`/`logical_edges` is allowed because it aligns runtime objects with the governance field without changing stored runtime fields.
...
2. A governance-shaped `reasoning_trace` can be accepted by runtime models and round-tripped through the exposed alias.
3. Legacy `confidence` and `confidence_interval` inputs still parse while they are in the compatibility window.
...
- Emits `logical_trace`/`logical_edges` as the only governance Forecast Object trace shape.
